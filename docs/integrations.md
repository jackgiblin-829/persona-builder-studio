...
## Prompt-taxonomy workbook

There is no prompt execution or tracking integration in this release. The Prompt Taxonomy tab exports a six-sheet Excel workbook containing the client brief, topic architecture, complete prompt library, Profound-ready import, competitor tracking plan, and entity watchlist. Prompt rows include persona, search theme, related prompt ID, search intent, prompt text, brand mode, topic class, archetype, business line, buyer context, quality score, market, language, evidence references, research snapshot, generation date, and generation mode. Formula-leading cells are protected for spreadsheet safety. Mock-origin libraries are clearly labeled; production exports require a complete, quality-passed library for every active persona.
