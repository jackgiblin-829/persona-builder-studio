...
import { TOPIC_CLASSES, type PromptStrategy, type TopicClass } from "@/contracts/prompt-strategy";
...
export const promptStrategyInputSchema = z
  .object({
    ...
    targetPromptCount: z.coerce.number().int().min(12).max(100),
    brandEntityTarget: z.coerce.number().int().min(0).max(100),
    categoryDiscoveryTarget: z.coerce.number().int().min(0).max(100),
    competitiveComparisonTarget: z.coerce.number().int().min(0).max(100),
    buyerEducationTarget: z.coerce.number().int().min(0).max(100),
    reputationRiskTarget: z.coerce.number().int().min(0).max(100),
    productLineTarget: z.coerce.number().int().min(0).max(100),
    personaPromptTargets: z.string().max(4000),
  })
  .superRefine((input, ctx) => {
    const topicTotal =
      input.brandEntityTarget +
      input.categoryDiscoveryTarget +
      input.competitiveComparisonTarget +
      input.buyerEducationTarget +
      input.reputationRiskTarget +
      input.productLineTarget;
    if (topicTotal !== input.targetPromptCount) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["targetPromptCount"],
        message: `Topic targets add up to ${topicTotal}; they must equal the total prompt target.`,
      });
    }
    if (input.competitiveComparisonTarget > 0 && !input.competitors.length) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["competitors"],
        message: "Add a competitor or set competitive comparisons to zero.",
      });
    }
  });
...
function parsePersonaTargets(value: string) {
  const targets: Record<string, number> = {};
  for (const [index, line] of value.split(/\n+/).entries()) {
    const trimmed = line.trim();
    if (!trimmed) continue;
    const match = trimmed.match(/^([a-z0-9]+(?:-[a-z0-9]+)*)\s*=\s*(\d+)$/);
    if (!match) {
      throw new ValidationError(
        `Persona target line ${index + 1} must use persona-slug=count, for example founder-ceo=20.`,
      );
    }
...
function strategyFromInput(input: PromptStrategyInput): PromptStrategy {
  const topicTargets: Record<TopicClass, number> = {
    brand_entity_authority: input.brandEntityTarget,
    unbranded_category_discovery: input.categoryDiscoveryTarget,
    competitive_comparison: input.competitiveComparisonTarget,
    buyer_education: input.buyerEducationTarget,
    reputation_risk: input.reputationRiskTarget,
    product_line_use_cases: input.productLineTarget,
  };
  return {
    ...
    topicTargets: Object.fromEntries(
      TOPIC_CLASSES.map((topic) => [topic, topicTargets[topic]]),
    ) as Record<TopicClass, number>,
    personaPromptTargets: parsePersonaTargets(input.personaPromptTargets),
  };
}
  ...
  return Promise.all(
    rows.map(async (project) => {
      ...
      return {
        ...project,
        organizationName:
          organizations.find((item) => item.organizationId === project.organizationId)
...
export async function getProjectWorkflowSummary(ctx: ProjectContext) {
  const project = await getProject(ctx);
  const [sources, activePersonas, promptSetRows, runRows, researchBriefRows] = await Promise.all([
    db
      .select()
      .from(dataSources)
